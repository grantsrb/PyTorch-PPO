...
class Model(nn.Module):
    def __init__(self, input_space, output_space, enc_size=56, env_type='snake-v0', view_net_input=False):
        ...
        self.enc_step1 = nn.Linear(input_space[-1], 300)
        self.enc_step2 = nn.Linear(input_space[-1], 100)
        self.enc_step3 = nn.Linear(input_space[-1], self.enc_size)
        self.encoder = nn.Sequential(self.enc_step1, self.enc_step2, self.enc_step3)
    ...
    def pred_action(self, state1, state2):
        enc1 = self.encoder(state1)
        enc2 = self.encoder(state2)
